    ...
    public function store($user, $data)
{
    ...
    $this->processBookingData($data, $immediatetime, $response, $consumer_type, $cuser);
...
private function validateBookingData($data)
{
    ...
    if (!isset($data['from_language_id'])) {
        $response = $this->setFailResponse($response, "Du måste fylla in alla fält", "from_language_id");
    } elseif ($data['immediate'] == 'no' && (!$data['due_date'] || !$data['due_time'] || !isset($data['customer_phone_type']))) {
        $response = $this->setFailResponse($response, "Du måste fylla in alla fält", "due_date");
    } elseif (!$data['duration']) {
        $response = $this->setFailResponse($response, "Du måste fylla in alla fält", "duration");
    }

    return $response;
}

private function setFailResponse($response, $message, $fieldName)
{
...
private function processBookingData($data, $immediatetime, &$response, $consumer_type, $cuser)
{
    ...
    if ($data['immediate'] == 'yes') {
        ...
        if ($due_carbon->isPast()) {
            $response = $this->setFailResponse($response, "Can't create booking in past", null);
            return;
        }
    }
    ...
    $this->finalizeResponse($response, $job);
}

private function setCertifiedAndGender(&$data)
{
    ...
    } elseif (in_array('normal', $data['job_for']) && in_array('certified_in_health', $data['job_for'])) {
        $data['certified'] = 'n_health';
    }
}

private function finalizeResponse(&$response, $job)
{
    $response['id'] = $job->id;
    $data['job_for'] = [];
